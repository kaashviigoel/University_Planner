...
def manage_subject(subject_id):
    with get_db() as conn:
        ...
        if request.method == 'PUT':
            ...
            conn.commit()
            c.execute("SELECT COUNT(*) FROM assignments WHERE subject_id = ?", (subject_id,))
            remaining_assignments = c.fetchone()[0]
            if remaining_assignments == 0:
                print(f"✅ Subject {subject_id} deleted + all related data cascaded!")
...
def timetable():
    with get_db() as conn:
        ...
        if request.method == 'POST':
            data = request.json
            c.execute("INSERT INTO timetable (day, time_slot, subject_id, room) VALUES (?, ?, ?, ?)",
                      (data['day'], data['time_slot'], data['subject_id'], data['room']))
            conn.commit()
        
        c.execute('''SELECT t.id, t.day, t.time_slot, t.room, s.code, s.name 
                     FROM timetable t JOIN subjects s ON t.subject_id = s.id 
                     ORDER BY t.day, t.time_slot''')
        timetable_list = []
        for row in c.fetchall():
            timetable_list.append({
                'id': row[0], 'day': row[1], 'time_slot': row[2], 'room': row[3],
                'subject_code': row[4], 'subject_name': row[5]
            })
        return jsonify(timetable_list)

@app.route('/api/timetable/<int:timetable_id>', methods=['PUT', 'DELETE'])
def manage_timetable(timetable_id):
    with get_db() as conn:
        c = conn.cursor()
        if request.method == 'PUT':
            data = request.json
            c.execute("UPDATE timetable SET day=?, time_slot=?, subject_id=?, room=? WHERE id=?",
                      (data['day'], data['time_slot'], data['subject_id'], data['room'], timetable_id))
            conn.commit()
        elif request.method == 'DELETE':
            c.execute("DELETE FROM timetable WHERE id=?", (timetable_id,))
            conn.commit()
        return jsonify({'message': 'Success'})

@app.route('/api/weekly-timetable')
def weekly_timetable():
    with get_db() as conn:
        c = conn.cursor()
        c.execute('''SELECT t.day, t.time_slot, s.code 
                     FROM timetable t JOIN subjects s ON t.subject_id = s.id 
                     ORDER BY 
                     CASE t.day 
